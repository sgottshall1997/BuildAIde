...
function loadPastProjects(): PastProject[] {
  try {
    const filePath = path.join(__dirname, 'pastProjects.json');
...
function calculateSimilarityScore(project: PastProject, criteria: {
  ...
  if (project.projectType.toLowerCase() === criteria.projectType.toLowerCase()) {
    score += 40;
  }
  ...
  const mappedQuality = finishLevelMap[criteria.materialQuality.toLowerCase()] || 'standard';
  if (project.finishLevel.toLowerCase() === mappedQuality.toLowerCase()) {
    score += 10;
  }
