...
import {
    PropertyFlipAnalysis,
    MarketAnalysis,
} from '../../interfaces/property-analysis/propertyAnalysis.interface';
...
Provide a comprehensive flip analysis including renovation scope, market positioning, and realistic profit projections.`
    ...
    async analyzeMarketTrends(zipCode: string, properties: any[]): Promise<MarketAnalysis> {
        try {
            const typesSet = new Set(properties.map(p => p.propertyType));
            const propertyTypes = Array.from(typesSet).join(', ');
            const analysis = await openai.chat.completions.create({
                model: "gpt-4o", // the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
                messages: [
                    {
