...
export async function getAIFlipOpinion(propertyData: {
    ...
    estimatedProfitAfterFees?: string;
...
export async function generatePreEstimateSummary(formData: any): Promise<string> {
...
export async function generateRiskRating(estimateData: any): Promise<{ riskLevel: 'low' | 'medium' | 'high'; riskExplanation: string; }> {
    const OpenAI = (await import("openai")).default;
    const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
...
export async function generatePastProjectInsight(currentProject: any, similarProjects: any[]): Promise<string> {
...
export async function generateSpenceTheBuilderResponse(message: string, estimateData: any, chatHistory: any[]): Promise<string> {
...
export async function generateVisualPreview(projectData: any): Promise<string> {
    const openai = require("openai");
    const client = new openai.OpenAI({ apiKey: process.env.OPENAI_API_KEY });
...
export async function generateHiddenCostInsights(estimateData: any): Promise<any> {
    const openai = require("openai");
    const client = new openai.OpenAI({ apiKey: process.env.OPENAI_API_KEY });
...
export async function generateClientEmail(estimateData: any): Promise<string> {
    const OpenAI = require("openai");
    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
...
export async function generateCostBreakdownExplanation(costBreakdown: any, projectType: string, estimatedCost: number): Promise<string> {
...
${costBreakdownJson}`;
        ...
        console.error("OpenAI API error:", error);
        throw new Error(`Failed to generate cost breakdown explanation: ${error || ""}`);
    }
}

export async function generateCategoryDetail(category: string, projectType: string, amount: number, percentage: number): Promise<string> {
    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
...
export async function processConversationalEstimator(userInput: string, currentEstimate: any, chatHistory: any[]): Promise<any> {
    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
...
export async function generatePersonalizedClientMessage(estimateData: any, clientName: string, projectLocation: string, messageType: string): Promise<string> {
    const openai = require("openai");
    const client = new openai.OpenAI({ apiKey: process.env.OPENAI_API_KEY });
