...
Format as a complete email with subject line.`;
  ...
  // Real Estate Property Search API with RealtyMole Integration
  app.post("/api/search-properties", async (req, res) => {
    try {
      const { zipCode, minPrice, maxPrice, minSqft, maxSqft, propertyTypes } = req.body;
  ...
  // Helper functions for property enhancement
  function determineProjectType(recommendations: string[]): string {
  ...
  function determineCrmStatus(flipScore: number): 'cold' | 'warm' | 'hot' {
    if (flipScore >= 80) return 'hot';
    if (flipScore >= 60) return 'warm';
    return 'cold';
  }

  function generateTags(flipAnalysis: any): string[] {
    const tags = [];
